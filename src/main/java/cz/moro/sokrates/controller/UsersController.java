...
public class UsersController {
	...
	public String processCreationForm(@ModelAttribute("user") @Valid User user, BindingResult result, SessionStatus status) {
	...
	@RequestMapping(value = "/admin/users/edit/{id}", method = {RequestMethod.PUT, RequestMethod.POST})
	public String processUpdateForm(@PathVariable Integer id, @ModelAttribute("user") User user, BindingResult result, SessionStatus status) {
		user.setId(id);
		new UserValidator().validate(user, result);
        if (result.hasErrors()) {
            return "users/addEditUser";
